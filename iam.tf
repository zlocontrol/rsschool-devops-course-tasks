...
// Получим свой AWS Account ID, чтобы строить ARN
...
// Политика на запись kubeconfig в SSM
resource "aws_iam_policy" "ssm_write_policy" {
  name        = "${var.name_prefix}-ec2-ssm-write-policy"
  description = "Allows EC2 instances to write kubeconfig into SSM Parameter Store"
  policy = jsonencode({
    Version = "2012-10-17",
    Statement = [
      {
        ...
        // Только на наш путь /<name_prefix>/kubeconfig
        Resource = "arn:aws:ssm:${var.aws_region}:${data.aws_caller_identity.current.account_id}:parameter/${var.name_prefix}/kubeconfig"
      }
    ]
  })
}

// Приклеим политику на запись к роли EC2
resource "aws_iam_role_policy_attachment" "k3s_ec2_role_ssm_write_attach" {
