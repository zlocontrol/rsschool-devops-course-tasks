...
resource "aws_instance" "k3s_master" {
  ...
  user_data = templatefile("${path.module}/install_k3s_master.sh.tpl", {
    ...
    #    PARAM_NAME                 = "/${var.name_prefix}/kubeconfig"
  })
...
resource "aws_instance" "gha_runner" {
  ...
  user_data = templatefile("${path.module}/install_runner.sh.tpl", {
    github_url   = var.github_url,
    runner_token = var.runner_token,
    RUNNER_NAME  = "gha-selfhosted-runner",
    repo_owner   = var.repo_owner,
    OWNER_REPO   = "${var.repo_owner}/${var.repo_name}"
